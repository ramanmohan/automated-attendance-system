'''
@author: Rajesh_Raman Implement  Dlib Face alignment strategy

However, this method/approach doesn't deform the original image like Dlib does.

This also categorizes the face in 3 types: Center, Left, Right, Top ,Bottom
...
class AlignCustom(object):
    ...
    def getPos(self, points):
        if abs(points[0] - points[2]) / abs(points[1] - points[2]) > 2:
            ...
            return "Left";
        elif abs(points[5] - points[7]) / abs(points[8] - points[7]) > 2:
            return "Top";
        elif abs(points[8] - points[7]) / abs(points[5] - points[7]) > 2:
            return "Bottom";
        return "Center"
