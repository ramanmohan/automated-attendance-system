...
class FaceFeature(object):
    def __init__(self, face_rec_graph, model_path = 'models/model-20170512-110547.ckpt-250000'):
        ...
        with face_rec_graph.graph.as_default():
            ...
            self.x = tf.placeholder('float', [None,160,160,3], name = 'batch_join'); #default input for the NN is 160x160x3
            self.embeddings = tf.nn.l2_normalize(
                                        resnet.inference(self.x, 0.6, phase_train=False)[0], 1, 1e-10, name = 'embeddings'); #some magic numbers that u dont have to care about
